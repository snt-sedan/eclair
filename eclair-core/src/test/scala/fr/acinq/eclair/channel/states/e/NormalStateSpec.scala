...
class NormalStateSpec extends TestkitBaseClass with StateTestsHelperMethods {
  ...
  test("recv UpdateAddHtlc (expiry too small)") { case (_, bob, alice2bob, bob2alice, _, bob2blockchain, _) =>
    within(30 seconds) {
      val tx = bob.stateData.asInstanceOf[DATA_NORMAL].commitments.localCommit.publishableTxs.commitTx.tx
      val htlc = UpdateAddHtlc("00" * 32, 0, 150000, expiry = 1, paymentHash = BinaryData("42" * 32), onionRoutingPacket = defaultOnion)
      alice2bob.forward(bob, htlc)
      val error = bob2alice.expectMsgType[Error]
  ...
  test("recv UpdateAddHtlc (value too small)") { case (_, bob, alice2bob, bob2alice, _, bob2blockchain, _) =>
    within(30 seconds) {
      val tx = bob.stateData.asInstanceOf[DATA_NORMAL].commitments.localCommit.publishableTxs.commitTx.tx
      val htlc = UpdateAddHtlc("00" * 32, 0, 150, expiry = 400144, paymentHash = BinaryData("42" * 32), onionRoutingPacket = defaultOnion)
      alice2bob.forward(bob, htlc)
      val error = bob2alice.expectMsgType[Error]
  ...
  test("recv UpdateAddHtlc (insufficient funds)") { case (_, bob, alice2bob, bob2alice, _, bob2blockchain, _) =>
    within(30 seconds) {
      val tx = bob.stateData.asInstanceOf[DATA_NORMAL].commitments.localCommit.publishableTxs.commitTx.tx
      val htlc = UpdateAddHtlc("00" * 32, 0, Long.MaxValue, expiry = 400144, paymentHash = BinaryData("42" * 32), onionRoutingPacket = defaultOnion)
      alice2bob.forward(bob, htlc)
      val error = bob2alice.expectMsgType[Error]
